...
function fmt(d) { return d.getDate()+'/'+d.toISOString().split('T')[0].slice(5,7) }
function fmtFull(d) { return d.getDate()+'/'+d.toISOString().split('T')[0].slice(5,7)+'/'+d.getFullYear() }

export default function Dashboard({ admin, commune, pharmaciesInit, weeks4, today }) {
  const [tab, setTab]             = useState('overview')
  const [sidebarOpen, setSidebar] = useState(false)
  const [pharmacies, setPharms]   = useState(pharmaciesInit)
  const [editModal, setEditModal] = useState(null)   // pharmacie en cours d'édition
  const [addForm, setAddForm]     = useState({ name:'', address:'', phone:'', lat:'', lng:'' })
  ...
  async function addPharmacy(e) {
    e.preventDefault()
    const r = await fetch('/api/admin/pharmacies', { method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify(addForm) })
    if (r.ok) { setAddForm({ name:'', address:'', phone:'', lat:'', lng:'' }); await loadPharms(); flash('Pharmacie ajoutée !') }
    else flash('Erreur lors de l\'ajout')
  }

  async function saveEdit(e) {
    e.preventDefault()
    const r = await fetch('/api/admin/pharmacies', { method:'PUT', headers:{'Content-Type':'application/json'},
      body: JSON.stringify(editModal) })
    if (r.ok) { setEditModal(null); await loadPharms(); flash('Pharmacie modifiée !') }
    else flash('Erreur lors de la modification')
  }
  ...
  async function planWeek(e) {
    e.preventDefault()
    if (!planForm.pharm_ids.length) { flash('Cochez au moins une pharmacie'); return }
    ...
    else flash('Erreur lors de la planification')
  }
  ...
  const navItem = (tabId, icon, label) => (
    <button className={`nav-item${tab===tabId?' active':''}`} onClick={() => { setTab(tabId); if(window.innerWidth<=900) setSidebar(false) }}>
      <i className={`fas fa-${icon}`}></i> {label}
    </button>
  )

  const initials = (admin.full_name || admin.username || 'A')[0].toUpperCase()
  ...
  return (
    <>
      ...
      <style>{`
        :root{--primary:#00AEEF;--primary-dark:#0077b6;--primary-light:#e0f7ff;--success:#10b981;--warning:#f59e0b;--danger:#ef4444;--text:#1f2937;--text-light:#6b7280;--bg:#f0f9ff;--shadow:0 4px 16px rgba(0,174,239,.12);--radius:14px}
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        body{font-family:'Poppins',sans-serif;background:var(--bg);color:var(--text);display:flex;min-height:100vh}
        .sidebar{width:240px;background:linear-gradient(180deg,var(--primary),var(--primary-dark));color:white;padding:1.5rem;position:fixed;height:100%;z-index:900;display:flex;flex-direction:column;gap:.2rem;transition:.3s}
        ...
        .commune-badge{background:rgba(255,255,255,.2);border-radius:8px;padding:.25rem .6rem;font-size:.72rem;margin-top:.25rem;display:block;text-align:center}
        .sidebar-prof{padding:.75rem;background:rgba(255,255,255,.12);border-radius:12px;display:flex;align-items:center;gap:.75rem;margin-bottom:.75rem;cursor:default}
        ...
        .sidebar-footer{margin-top:auto;display:flex;flex-direction:column;gap:.2rem}
        ...
        input,select{padding:.6rem .85rem;border:2px solid var(--primary-light);border-radius:8px;font-family:'Poppins',sans-serif;font-size:.85rem;color:var(--text);outline:none;transition:.2s}
        input:focus,select:focus{border-color:var(--primary)}
        .btn{padding:.5rem .9rem;border:none;border-radius:8px;font-weight:600;cursor:pointer;font-family:'Poppins',sans-serif;font-size:.82rem;display:inline-flex;align-items:center;gap:.4rem;transition:.2s;text-decoration:none;white-space:nowrap}
        ...
        table{width:100%;border-collapse:collapse;background:white;border-radius:12px;overflow:hidden;font-size:.84rem}
        thead{background:linear-gradient(135deg,var(--primary),var(--primary-dark));color:white}
        th,td{padding:10px 13px;text-align:left}
      ...
      <aside className={`sidebar${sidebarOpen?' open':''}`}>
        ...
        <div className="sidebar-prof">
          ...
          <div>
            <div className="prof-name">{admin.full_name || admin.username}</div>
      ...
      <main className="main">
        ...
        <div className="topbar">
          <div style={{display:'flex',alignItems:'center',gap:'1rem'}}>
            <button className="burger" onClick={() => setSidebar(o => !o)}>
              <span></span><span></span><span></span>
            </button>
            <div className="topbar-title">
              {{ overview:'Tableau de bord', pharmacies:'Pharmacies', planning:'Planning de garde' }[tab]}
            </div>
          </div>
                  ...
                  <table>
                    ...
                    <tbody>
                      {todayGuards.nuit.map(p => (
                        <tr key={p.id}>
                          <td><strong>{p.name}</strong><br /><small style={{color:'var(--text-light)'}}>{p.address||''}</small></td>
                          <td>{p.phone ? <a href={`tel:${p.phone}`} style={{color:'var(--primary)'}}>{p.phone}</a> : '—'}</td>
                    ...
                    <tbody>
                      {todayGuards.dimanche.map(p => (
                        <tr key={p.id}>
                          ...
                          <td>{p.phone ? <a href={`tel:${p.phone}`} style={{color:'var(--warning)'}}>{p.phone}</a> : '—'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <table>
                  ...
                  <tbody>
                    {pharmacies.map(p => (
                      <tr key={p.id}>
                        <td><strong>{p.name}</strong><br /><small style={{color:'var(--text-light)'}}>{p.address||''}</small></td>
                        <td style={{fontSize:'.8rem',color:'var(--primary-dark)'}}>{p.doctor||'—'}</td>
                        <td style={{fontSize:'.8rem'}}>{p.phone ? <a href={`tel:${p.phone}`} style={{color:'var(--primary)'}}>{p.phone}</a> : '—'}</td>
                        <td><span className={`badge ${p.is_active ? 'badge-green' : 'badge-red'}`}>{p.is_active ? 'Active' : 'Inactive'}</span></td>
                        <td>
                          <div style={{display:'flex',gap:'.4rem'}}>
                            <button className="btn btn-sm btn-primary" onClick={() => setEditModal({...p, lat: p.latitude||'', lng: p.longitude||''})}>
                              <i className="fas fa-edit"></i>
                            </button>
                            <button className="btn btn-sm btn-danger" onClick={() => deletePharmacy(p.id, p.name)}>
                ...
                <div className="form-grid" style={{marginBottom:'1rem'}}>
                  ...
                  <div className="form-group">
                    ...
                    <select value={planForm.garde_type} onChange={e=>setPlanForm(f=>({...f,garde_type:e.target.value}))}>
                      ...
                      <option value="dimanche">☀️ Dimanche uniquement</option>
                    </select>
              ...
              <div className="form-grid">
                ...
                <div className="form-group" style={{justifyContent:'flex-end',paddingTop:'1rem'}}>
                  <label style={{display:'flex',alignItems:'center',gap:'.5rem',cursor:'pointer'}}>
                    <input type="checkbox" checked={!!editModal.is_active} onChange={e=>setEditModal(m=>({...m,is_active:e.target.checked}))} /> Active
                  </label>
                </div>
              </div>
...
export async function getServerSideProps({ req, res }) {
  ...
  const [communeRows, pharmaciesRows] = await Promise.all([
  ...
  for (const r of gardeRows) {
    ...
    if (!byDate[r.garde_date][key].find(x => x.id === r.pid)) {
      byDate[r.garde_date][key].push({ id: r.pid, name: r.name, phone: r.phone, latitude: r.latitude, longitude: r.longitude })
    }
  }

  // Construire 4 semaines
  const weeks4 = []
  for (let w = 0; w < 4; w++) {
  ...
  return {
    props: {
      admin:          { id: admin.id.toString(), username: admin.username, full_name: admin.full_name||'', role: admin.role, communeId: communeId.toString() },
      commune:        { id: commune.id.toString(), name: commune.name, slug: commune.slug },
